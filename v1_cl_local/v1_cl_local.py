...
import os 
import argparse # Para rodar programa pela linha de comando
from datetime import datetime # Para timestamp usado para caminho personalizado do relatório
...
def main(video_path):
    ...
    print('--Criando Lista de Frames--\n')
    frame_rate = 30
    frames = extract_frames(video_path=video_path, frame_rate=frame_rate)

    print('--Definindo Path para PDF--\n')
    video_dir = os.path.dirname(video_path)
    timestamp = datetime.now().strftime('%Y%m%d_%H%M%S') # Definindo path personalizado para evitar overwrite
    pdf_path = os.path.join(video_dir, f'results_{timestamp}.pdf')
    ...
    print('--Gerando Dicionário de Pessoas no Vídeo--\n')
    ...
    print('-- Abrindo PDF no VSCode --')
    open_pdf_in_vscode(pdf_caminho)

if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Processamento de vídeo e geração de relatório de emoções dos participantes do vídeo")
    parser.add_argument('video_path', type=str, help='Path para o vídeo a ser analisado')
